...
function ajax(options, args) {
    const self = this,
        ...
        renderDataObject = _.merge({}, data, optionData),
    ...
    if (options.params) {
        ...
        target.data = _.merge({}, mergeData, renderParams(options.params, renderDataObject));
    }
        ...
        ((err ? opts.errorActions : opts.nextActions) || []).forEach((item) => {
            if (err && opts.errorActions) {
                if (overrideErrorMessage) {
                    error.message = _.merge({}, error.message, overrideErrorMessage[error.statusCode] || {});
                }
            ...
            item.options = _.merge({}, response, item.options);
            createViewModel.call(self, item).action();
        });
    ...
    this._dataService.ajax(_.merge({}, target, { uri: uri }))    
        .catch(nextAction)
        .subscribe(data => nextAction(null, data));
}
